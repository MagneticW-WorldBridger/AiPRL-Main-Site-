...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || "postgres://neondb_owner:npg_THMlQu6ZWmD4@ep-weathered-dream-adbza7xj-pooler.c-2.us-east-1.aws.neon.tech/neondb?sslmode=require",
  ssl: {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.log(`[MEMORY] Processing request for user: ${userIdentifier}`);
    
    // Get or create conversation
    const conversationId = await getOrCreateConversation(userIdentifier);
    console.log(`[MEMORY] Using conversation: ${conversationId}`);
    
    // Get conversation history (last 30 messages for better context)
...
CRITICAL: Before EVERY response, read the ENTIRE conversation history carefully. If the user says they already gave you information, FIND IT in the history and acknowledge it. Never say "I don't have memory" when the information is in the current conversation.`
    ...
    console.log(`[MEMORY] Sending ${messages.length} messages to OpenAI`);
    ...
    console.log(`[MEMORY] Saved conversation messages to database`);
